...
export default function PublicChallanPage() {
  ...
  useEffect(() => {
    if (!bill_id) return;
    const fetchPublicChallan = async () => {
      try {
        setLoading(true);
        const res = await axios.get(`/api/challans/public/${bill_id}/`);
        if (res.data.success) {
          setData(res.data.data);
        } else {
          setError(res.data.message || "Failed to load challan.");
        }
      } catch (err) {
        setError(err.response?.data?.message || "Challan document not found or sharing has been disabled.");
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-slate-50 dark:bg-[#0B1220] p-4 text-center">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 max-w-sm mb-6">
          {error || "Could not retrieve the requested challan details."}
        </p>
        <button
  ...
  const { invoice: challan, contact, company } = data;
  const activeGstType = challan?.internal_note?.startsWith("[GST_TYPE:inter]") ? "inter" : "intra";
        ...
        <div id="printable-invoice" className="bg-white text-slate-800 p-8 rounded-2xl border border-slate-250 dark:border-slate-850 dark:bg-slate-900 dark:text-slate-100 shadow-xl space-y-6">
          ...
          <div className="flex justify-between items-start border-b border-slate-200 dark:border-slate-800 pb-6">
            ...
            <div className="text-right space-y-1 text-sm font-medium">
              ...
              <p className="text-slate-900 dark:text-slate-100 font-extrabold">Bill ID: {challan?.bill_id}</p>
              <p className="text-slate-600 dark:text-slate-400">Challan #: {challan?.invoice_number || "N/A"}</p>
              <p className="text-slate-600 dark:text-slate-400">Date: {challan?.invoice_date}</p>
            </div>
          </div>
            ...
            <div id="printable-invoice-info-grid-right" className="md:border-l md:border-slate-200 dark:md:border-slate-800 md:pl-6 space-y-3">
              ...
              <div className="text-sm font-medium">
                <div>
                  ...
                  <p className="text-slate-800 dark:text-slate-200 font-bold mt-0.5">{challan?.party_challan_no || "N/A"}</p>
                </div>
          ...
          <div id="printable-invoice-bottom-flex" className="flex flex-col md:flex-row justify-between items-start pt-4 gap-6">
            ...
            <div className="flex-1 space-y-4 max-w-md w-full">
              {challan?.notes && (
                <div className="bg-slate-50 dark:bg-slate-800/20 p-4 rounded-xl border border-slate-100 dark:border-slate-800/60">
                  ...
                  <p className="text-xs text-slate-600 dark:text-slate-400 whitespace-pre-wrap leading-relaxed">{challan?.notes}</p>
                </div>
              )}
            ...
            <div className="w-full md:w-80 bg-slate-50 dark:bg-slate-850/40 border border-slate-200 dark:border-slate-800 p-5 rounded-2xl space-y-3 shadow-sm text-sm">
              ...
              {(() => {
                const gstSum = challan?.gst_summary || {};
